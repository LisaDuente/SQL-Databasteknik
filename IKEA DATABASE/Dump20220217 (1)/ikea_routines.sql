...
/*!50001 VIEW `contactinfoemployees` AS select `p`.`PersonalName` AS `PersonalName`,`p`.`PersonalSurname` AS `PersonalSurname`,`p`.`Phone` AS `Phone`,`p`.`Email` AS `Email`,`d`.`DepartmentName` AS `DepartmentName`,`p`.`Position` AS `Position` from (`personal` `p` left join `departments` `d` on((`p`.`DepartmentID` = `d`.`DepartmentID`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `quantityinwarehouse` AS select `p`.`ProductID` AS `ID`,`p`.`ProductName` AS `ProductName`,sum(`ws`.`Quantity`) AS `TotalQuantity` from (`products` `p` left join `warehousestatus` `ws` on((`p`.`ProductID` = `ws`.`ProductID`))) group by `p`.`ProductID` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
VALUES(CID, AES_ENCRYPT(CONCAT(CIBAN,CID), "123Test"),AES_ENCRYPT(CONCAT(CBIC,CID), "123Test"),
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertCustomer`(CName VARCHAR(255), CSurName VARCHAR(255), CStreet VARCHAR(255), CHouseNr INT, CPostCode INT, CCity VARCHAR(255), CCountry VARCHAR(255), CPhone VARCHAR(255), CMail VARCHAR(255))
ADProcess: BEGIN
...
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	INSERT INTO Customers 
		VALUES(NULL, CNAME, (SELECT ID FROM adressView AS DA WHERE DA.StreetName = CStreet AND DA.HouseNR = CHouseNR AND DA.PostCode = CPostCode AND CityName = CCity AND DA.CountryName = CCountry GROUP BY ID), CPhone, CMail, CSurName);
        LEAVE ADProcess;
ELSE
	...
	ELSE
        ...
        INSERT INTO Customers VALUES (NULL, CName, @tempAdressID, CPhone, CMail,CSurName);
        LEAVE ADProcess;
	END IF;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertPersonal`(PName VARCHAR(255), PSurname VARCHAR(255), CStreet VARCHAR(255), CHouseNr INT, CPostCode INT, CCity VARCHAR(255), CCountry VARCHAR(255), PDepartment INT, PPhone VARCHAR (255), PEmail VARCHAR (255), PPosition VARCHAR(255), PSalary INT)
ADProcess: BEGIN

-- if everything exists
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
    INSERT INTO Personal
		VALUES(NULL, PName, (SELECT ID FROM adressView AS DA WHERE DA.StreetName = CStreet AND DA.HouseNR = CHouseNR AND DA.PostCode = CPostCode AND CityName = CCity AND DA.CountryName = CCountry GROUP BY ID), PDepartment, PPhone, PEmail, PPosition, PSalary, PSurname);
        LEAVE ADProcess;
ELSE
    -- check the countryID, if its not in the databank, leave the process
	IF EXISTS(SELECT CountryName FROM Countries WHERE CountryName = CCountry) THEN
	...
	ELSE
        ...
        SELECT @tempAdressID := createAdressID();
        INSERT INTO Adresses VALUES (@tempAdressID, @tempStreetID);
        INSERT INTO Personal VALUES (NULL, PName, @tempAdressID, PDepartment, PPhone, PEmail, PPosition, PSalary, PSurname);
        LEAVE ADProcess;
	END IF;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateCustomer`(CID INT, CStreet VARCHAR(255), CHouseNr INT, CPostCode INT, CCity VARCHAR(255), CCountry VARCHAR(255))
ADProcess: BEGIN
-- if everything exists
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	UPDATE Customers 
		SET AdressID = (SELECT ID FROM adressView AS DA WHERE DA.StreetName = CStreet AND DA.HouseNR = CHouseNR AND DA.PostCode = CPostCode AND CityName = CCity AND DA.CountryName = CCountry GROUP BY ID) WHERE CustomerID = CID;
	INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("C",CID));
	LEAVE ADProcess;
ELSE
	-- create new adress id?
    -- check the countryID, if its not in the databank, leave the process
	...
	ELSE
        ...
        SELECT @tempAdressID := createAdressID();
        INSERT INTO Adresses VALUES (@tempAdressID, @tempStreetID);
        UPDATE Customers SET AdressID = @tempAdressID WHERE CustomerID = CID;
        INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("C",CID));
        LEAVE ADProcess;
	END IF;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateDepartment`(DIP INT, CStreet VARCHAR(255), CHouseNr INT, CPostCode INT, CCity VARCHAR(255), CCountry VARCHAR(255))
ADProcess: BEGIN
-- if everything exists
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	UPDATE Departments 
		SET AdressID = (SELECT ID FROM adressView AS DA WHERE DA.StreetName = CStreet AND DA.HouseNR = CHouseNR AND DA.PostCode = CPostCode AND CityName = CCity AND DA.CountryName = CCountry GROUP BY ID) WHERE DepartmentID = DID;
	INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("D",DID));
	LEAVE ADProcess;
ELSE
	-- create new adress id?
    -- check the countryID, if its not in the databank, leave the process
	...
	ELSE
        ...
        SELECT @tempAdressID := createAdressID();
        INSERT INTO Adresses VALUES (@tempAdressID, @tempStreetID);
        UPDATE Departments SET AdressID = @tempAdressID WHERE DepartmentID = DID;
        INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("D",DID));
        LEAVE ADProcess;
	END IF;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `UpdateShippers` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	...
	INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("S",SID));
	LEAVE ADProcess;
ELSE
	-- create new adress id?
    -- check the countryID, if its not in the databank, leave the process
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `UpdateWarehouseAdress` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateWarehouseAdress`(WID INT, CStreet VARCHAR(255), CHouseNr INT, CPostCode INT, CCity VARCHAR(255), CCountry VARCHAR(255))
ADProcess: BEGIN
-- if everything exists
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM adressView WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	UPDATE Warehouse
		SET AdressID = (SELECT ID FROM adressView AS DA WHERE DA.StreetName = CStreet AND DA.HouseNR = CHouseNR AND DA.PostCode = CPostCode AND CityName = CCity AND DA.CountryName = CCountry GROUP BY ID) WHERE CustomerID = WID;
	INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("W",WID));
	LEAVE ADProcess;
ELSE
	-- create new adress id?
    -- check the countryID, if its not in the databank, leave the process
	...
	ELSE
        ...
        SELECT @tempAdressID := createAdressID();
        INSERT INTO Adresses VALUES (@tempAdressID, @tempStreetID);
        UPDATE Warehouse SET AdressID = @tempAdressID WHERE WarehouseID = WID;
        INSERT INTO TransactionHistory VALUES(NULL, 3, NOW(),CONCAT("W",WID));
        LEAVE ADProcess;
	END IF;
