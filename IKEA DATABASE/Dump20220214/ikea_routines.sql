...
IF EXISTS(SELECT StreetName, HouseNr, Postcode, CityName, CountryName FROM customeradresses WHERE StreetName = CStreet AND HouseNR = CHouseNR AND PostCode = CPostCode AND CityName = CCity AND CountryName = CCountry) THEN
	INSERT INTO Customers 
		VALUES(NULL, CNAME, (SELECT AdressID FROM Customers AS C LEFT JOIN Customeradresses AS CA ON C.CustomerID = CA.ID WHERE CA.StreetName = CStreet AND CA.HouseNR = CHouseNR AND CA.PostCode = CPostCode AND CityName = CCity AND CA.CountryName = CCountry GROUP BY AdressID), CPhone, CMail);
ELSE
...
-- Dump completed on 2022-02-14  9:43:32
