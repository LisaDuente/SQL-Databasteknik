...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `showorderstatus` AS select `c`.`CustomerName` AS `CustomerName`,`o`.`OrderID` AS `orderid`,`p`.`ProductName` AS `ProductName`,`od`.`Quantity` AS `Quantity`,`os`.`StatusName` AS `StatusName`,`o`.`ShippingDate` AS `ShippingDate` from ((((`customers` `c` left join `orders` `o` on((`c`.`CustomerID` = `o`.`CustomerID`))) left join `orderdetails` `od` on((`o`.`OrderID` = `od`.`OrderID`))) left join `orderstatus` `os` on((`o`.`OrderStatusID` = `os`.`StatusID`))) left join `products` `p` on((`od`.`ProductID` = `p`.`ProductID`))) where ((`o`.`OrderID` is not null) and (`os`.`StatusName` <> 'delivered')) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2022-02-10 14:51:30
